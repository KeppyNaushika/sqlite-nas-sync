...
describe('主キーとセカンダリUNIQUEに同時にぶつかるINSERT', () => {
  ...
  it('リモートが新しいとき、例外を抜けさせず1行へ畳む', () => {
    ...
    expect(
      rowsOf(db, `SELECT recordId, mergedInto FROM _tombstone`)
    ).toEqual([{ recordId: 'seat-9', mergedInto: 'seat-3' }]);
  });
  ...
  it('ローカル行が勝つ相手が居れば、更新対象の行の方を勝者へ畳む（届いた版を黙って捨てない）', () => {
    ...
    // 席3の子は席9へ引き取られる
    expect(
      rowsOf(db, `SELECT id, examStudentId FROM question_scores`)
    ).toEqual([{ id: 'qs-3', examStudentId: 'seat-9' }]);
    expect(result.folds).toEqual([
      {
        tableName: 'exam_students',
        losingId: 'seat-3',
  ...
  it('両端末が同じ1行へ揃い、どちらの子も失われない', async () => {
    ...
    const snapshot = (db: Database.Database): string =>
      JSON.stringify([
        ...
        rowsOf(
          db,
          `SELECT id, examStudentId FROM question_scores ORDER BY id`
        ),
      ]);
...
describe('公開APIを直接呼んだときの「書く列が無い」', () => {
  ...
  it('主キーとタイムスタンプだけの表は、列を正しく渡せば普通に書ける', () => {
    const db = new Database(':memory:');
    db.exec(
      `CREATE TABLE pk_ts (id TEXT PRIMARY KEY, updatedAt TEXT NOT NULL)`
    );
