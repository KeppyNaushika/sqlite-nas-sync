...
describe('セカンダリUNIQUE違反で負けた行の子の引き取り', () => {
  ...
  it('負けた行の子が消えず、勝った行の子として両端末に残る', async () => {
    ...
    for (const [label, db] of [
      ...
      expect(
        idsOf(db, `SELECT id FROM exam_students`),
        `client-${label}`
      ).toEqual(['es-b'])
      // 採点は両方とも残り、勝った生徒にぶら下がる
      expect(
        idsOf(db, `SELECT id FROM question_scores`),
        `client-${label}`
      ).toEqual(['qs-a', 'qs-b'])
      const parents = (
        db
          .prepare(`SELECT DISTINCT examStudentId AS id FROM question_scores`)
          .all() as {
          id: string
        }[]
      ).map((row) => row.id)
      ...
      // 孫（採点のメモ）も道連れになっていない
      expect(
        idsOf(db, `SELECT id FROM score_notes`),
        `client-${label}`
      ).toEqual(['note-a'])
    }
  ...
  it('勝った側の同期が詰まらず、同じ相手の無関係な変更も届く', async () => {
    ...
    const firstPull = await performSync(
      dbB,
    ...
    dbA
      ...
      .run('memo-1', '競合とは無関係な変更', '2024-07-01T00:00:00Z')
    await performSync(dbA, makeConfig(pathA, 'client-a'), TABLES)
    ...
    const snapshot = (db: Database.Database): string =>
      JSON.stringify([
        ...
        db
          ...
          .all(),
        db
          .prepare(`SELECT id, questionScoreId FROM score_notes ORDER BY id`)
          .all(),
      ])
  ...
  it('フルマージ経路（changelogギャップ）でも負けた行の子が消えない', async () => {
    ...
    expect(
      fullMerge.warnings.filter((warning) => warning.includes('FOREIGN KEY'))
    ).toEqual([])
  ...
  it('付け替えた子が子自身のユニークにぶつかる場合もLWWで畳まれ、その孫も引き取られる', async () => {
    ...
    for (const [label, db] of [
      ...
      const noteParents = (
        db
          .prepare(`SELECT DISTINCT questionScoreId AS id FROM score_notes`)
          .all() as {
          id: string
        }[]
      ).map((row) => row.id)
