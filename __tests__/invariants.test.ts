...
function findLines(
  ...
  for (const file of sourceFiles()) {
    file.text.split('\n').forEach((line, index) => {
      ...
      if (predicate(line)) hits.push({ where: `${file.path}:${index + 1}`, line: code });
    });
  }
  return hits;
}
...
describe('規則: 帳簿（`_id_merge` / `_tombstone`）を id で引くときは、綴り違いに備える', () => {
  it('1件引きの文は、大小を畳んだうえで新しい方を採る', () => {
    ...
    for (const file of sourceFiles()) {
      ...
      const statements = file.text.match(/`[^`]*FROM (?:_id_merge|_tombstone)[^`]*`/g);
      for (const statement of statements ?? []) {
        ...
        if (!statement.includes('COLLATE NOCASE')) {
          offenders.push(`${file.path}: 表名は COLLATE NOCASE で引くこと\n${statement}`);
        }
    ...
    expect(checked, '1件引きの文が1つも見つからない＝この検査が空振りしている').toBeGreaterThan(0);
  });
  ...
  it('`_tombstone.mergedInto` を書くのは、判断を通る経路だけ', () => {
    ...
    const unexpected = [...new Set(writers)].filter((file) => !allowed.has(file));
    expect(
      unexpected,
...
describe('規則: 表名をキーに設定を引くときは大小を畳む', () => {
  ...
  it('正本（`makeTableConfigLookup`）は畳んで引いている', () => {
    ...
    const remote = fs.readFileSync(
      path.join(SRC, 'sync', 'remote.ts'),
      'utf8'
    );
    const body = remote.slice(remote.indexOf('export function makeTableConfigLookup'));
