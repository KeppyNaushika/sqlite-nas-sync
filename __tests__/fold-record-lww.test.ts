...
describe('規則2・規則3: 読み替えの適用条件と、読み替えた行が名乗る時刻', () => {
  ...
  it('席を別の行と争っているときは名乗らない（借りた時刻で勝つと収束しない）', () => {
    ...
    expect(db.prepare(`SELECT id, body FROM scores ORDER BY id`).all()).toEqual(
      [{ id: 's-b', body: '新しい方' }]
    )
  })
})
...
describe('子どうしの畳みが刻む時刻（親と子で時刻列の名前が違う場合）', () => {
  ...
  it('外部キーが子の主キーを兼ねる1:1で、席が空いている場合（idが動くだけ）', () => {
    ...
    expect(db.prepare(`SELECT id, detailId FROM detail_notes`).all()).toEqual([
      { id: 'note-1', detailId: 'p-b' },
    ])
  })
})
...
describe('畳み先の鎖を張り替えるとき', () => {
  it('張り替えても、その記録が確定した時刻は巻き戻らない', () => {
    ...
    expect(
      db
        .prepare(
          `SELECT losingId, winningId, mergedAt FROM _id_merge ORDER BY losingId`
        )
        .all()
    ).toEqual([
...
describe('旧バージョンが残した畳み先の鎖', () => {
  ...
  it('届いた畳み先が既に畳まれていても、敗者行を終端へ畳める', () => {
    ...
    db.prepare(
      `INSERT INTO m VALUES ('kid-1', 'A', '2026-01-02T00:00:00Z')`
    ).run()
    ...
    expect(db.prepare(`SELECT id FROM n ORDER BY id`).all()).toEqual([
      { id: 'B' },
    ])
    // 子は中間ではなく終端へ付け替わる
