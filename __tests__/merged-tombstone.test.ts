...
describe('畳み先つきtombstoneの伝播', () => {
  ...
  async function seedFoldedWorld(): Promise<{
    ...
    dbZ.prepare(`INSERT INTO parents (id, ukey, updatedAt) VALUES (?, ?, ?)`).run(
      'aaa',
      'k1',
  ...
  it('mergedInto列を持たない旧クライアントのtombstoneも従来どおり適用できる', async () => {
    ...
    const result = await performSync(dbNew, makeConfig(pathNew, 'client-new'), TABLES);
    expect(result.warnings.filter((warning) => warning.includes('Sync failed'))).toEqual([]);
