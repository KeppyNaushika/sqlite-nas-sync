...
戻り値には実行されたアクション（`inserted` / `upserted` / `skipped` 等）と、競合があった場合は
`ConflictInfo`（`resolution: 'local_wins' | 'remote_wins'` を含む）、畳みの一覧 `folds`
（`upserted` は**届いた行を実際に書いたとき**だけです。ローカルが勝って届いた行を採らなかった
場合は `skipped` になり、`conflict.resolution` が `local_wins` になります）、
（{@link RecordFold}）、および `warnings` が入ります。`warnings` には「向け直した先の親が
消えていたので `ON DELETE` に従った」（行を採らなかった／外部キーの列を `NULL` にした）が
載ります — [遅れて届いた子と、消えた畳み先](#遅れて届いた子と消えた畳み先) を参照。
`syncNow` 経由で使う場合は `SyncResult.warnings` にそのまま出ます。
...
**張り替えても記録の時刻は動かしません** — 向き先が変わっただけで「`bbb` が畳まれた時刻」は
変わらないからです。動かすと、対になる `_tombstone.deletedAt`（`bbb` 自身の畳みの時刻のまま）
との間で食い違い、敗者行が生きたままその子だけ終端へ読み替えられます。逆に、**あとから
届いた記録の畳み先が既に畳まれている**場合も、書く前に終端まで辿ります（`_tombstone.mergedInto`
は同期で渡るので、`A→C` と `C→B` の鎖がそのまま届きます）。
畳む向きが後から反転した場合に生まれる「自分自身を指す記録」は捨てられます。

**古いDBに残っている鎖は、起動時（`setupChangelog`）に一度だけ畳み直します。**
`_id_merge` は同期されないローカル索引なので、自分のDBを一度直せば、他の端末が古い
バージョンを積んでいても鎖は入ってきません。記録の時刻は動かしません。矛盾した記録
（`A→B` と `B→A` が同時に立つ循環）は、いちばん新しい主張だけを残します。畳み先は
`_id_merge` と `_tombstone.mergedInto` の2か所に載っているので、**畳み直しも刈り取りも
両方を揃えて動かします**（片方だけ直すと、読み替え表は「畳まれていない」と言うのに
tombstone は畳み先を名乗り続け、遅れて届いた子が消えた親を指したまま入ります）。
...
リモートの tombstone に mergedInto がある：
  ...
  ※ 畳み先の行がローカルに無ければリモートから読んで先に入れる。
    ...
    鎖を辿って終端が動き、その終端の行がどこにも無いときは **1. の記録も書かない**
    （書くと敗者行は生きたまま「畳まれた」と記録され、その子だけが手元に無い終端へ
    読み替えられる）
...
1. **決定論的に収束する** — どの端末で解決しても同じ答えになる（時刻のLWW、同点は主キーの
   辞書順）。端末ごとに違う情報は判断に使いません
2. **書かれたデータを黙って壊さない** — 書き込みを勝手に古く／新しく見せかけて、他端末の
   編集を消すようなことはしません
3. **解決できないことは、解決できないと報告する** — 黙って片方を選んだり、黙って捨てたり
   しません。`SyncResult.warnings` に出ます

**保証しないこと（ドメインの意味の判断）**

- 別idの2行が「同じもの」かどうか — 判断材料は**あなたが宣言したユニークキー**だけです
- 親を付け替えたとき子がどう追随すべきか
- 同一主キー・同時刻で中身が割れたとき、どちらが正しいか

**アプリが決めること**

- **報告をどう人へ見せるか。** `warnings` と `folds` は出しっぱなしにせず、利用者へ届けて
  ください（「小計が2つあったので1つにまとめました」「この行は端末間で食い違っています」）
- **親の付け替えを子へ波及させるか。** させたいなら、アプリ側で**子の `updatedAt` に触って
  ください**。ライブラリは子の時刻を勝手に動かしません（動かすと、その分だけ他端末の
  編集を消すためです）

### 畳みのふるまいを決めている2つの原則

以下の細かい規則は、すべてこの2文から出ています。**自分のスキーマで何が起きるかは、
この2文に当てはめれば予測できます。**

> **1. 同じ操作には1つの時刻しか無い。**
> その時刻は、突き合わせが起きる場で名乗り、LWW で全体へ伝わる。
>
> **2. 手元に居たら何が起きていたかを、そのまま再現する。**
> 再現の仕方を決めるのは、あなたのスキーマが宣言した `ON DELETE` です。

原則1 の帰結が「畳みの記録もLWWの下に置く」（次の節）、原則2 の帰結が
「遅れて届いた子と、消えた畳み先」（その次の節）です。

### 畳みの記録もLWWの下に置く（原則1の帰結）

`_tombstone`（畳み先）も `_id_merge`（読み替え表）も、**行と同じ「主張」**として扱います。
それが確立した時刻を持ち、新しい主張が勝ちます。**同期を回した時刻はどこにも書きません。**

| 記録 | 刻む時刻 |
| --- | --- |
| `_tombstone.deletedAt`（畳み） | **勝った行の `updatedAt`** |
| `_tombstone.deletedAt`（利用者操作による削除） | 削除した時刻（従来どおり） |
| `_id_merge.mergedAt` | **勝った行の `updatedAt`**（張り替え・畳み直しでも動かさない） |

畳みに現在時刻を刻むと、実データの `updatedAt` は必ずそれより過去になるため、その id は
「削除済みより古い挿入」として**無条件に止まります**。畳みは削除ではなく「この2行は同じ
ものだ」という判断なので、止めてよいのは**その判断が見ていた版まで**です。

この時刻を物差しに、次が決まります。

1. **敗者idの復活** — 敗者行がその後もっと新しい版で戻ってきたら（例: ユニークキーが
   変更されて、もう衝突しない）、その版は通ります。畳みの記録は**古い判断**として
   退けられ、行はどの端末でも生き返ります
2. **読み替えの適用条件** — 遅れて届いた子の外部キーを畳み先へ向け直すのは、
   **記録がローカルの敗者行より新しいとき**だけです。敗者行が記録より新しく更新されて
   いれば、その子は敗者を指したまま入ります
3. **読み替えても、行の時刻には触りません** — 向け直しで書き換わるのは**外部キーの列だけ**で、
   他の列は元の書き手のものです。行の時刻は1つしか無いので、行全体で畳みの時刻を名乗ると
   **外部キー以外の列について過大に申告**することになり、その分だけ
   「元の時刻〜畳みの時刻」の窓に入る**他端末の本物の編集が消えます**（実測）。

   名乗れば解ける食い違いはあります（同じ子を両端末が別々の親へ繋いだまま、同じ時刻で
   持ち合う形）。**それでも名乗りません** —— 書かれたデータを消してまで解くものでは
   ないからです。ライブラリはこれを**解かずに報告**します
   （[直っていないこと](#直っていないこと既知) 3）。**親の付け替えを子へ波及させたい
   場合は、アプリ側で子の `updatedAt` に触ってください。**

同じ1回の畳みには、**畳んだ端末も畳まなかった端末も同じ時刻**を刻みます。畳みを適用する
たびに手元の時刻へ寄せ替えると、端末を1つ経るごとにしきい値が現在へ近づき、まだ届いて
いない新しい版が捨てられる範囲が広がっていきます。

### 遅れて届いた子と、消えた畳み先（原則2の帰結）

畳み先が**そのあと利用者に消された**あとで、敗者の子が別の端末から届くことがあります。
向け直した先はもう居ないので、そのまま入れると外部キー違反になり、**その相手ぶんの
取り込みが丸ごと巻き戻って同期がその相手から永久に止まります**。

このとき、ライブラリは「**その子が手元に居たら何が起きていたか**」をそのまま再現します。
判断材料は、あなたのスキーマが宣言している `ON DELETE` です。

| `ON DELETE` | 遅れて届いた子の扱い |
| --- | --- |
| `CASCADE` | その子を採らない |
| `SET NULL`（対象列がすべて nullable） | **その外部キーの全列**を `NULL` にして採る |
| `SET NULL`（`NOT NULL` 列を含む） | 採らずに警告（実質 `RESTRICT`） |
| `SET DEFAULT`（既定値の親が居る／既定値に `NULL` を含む） | **その外部キーの全列**を既定値にして採る |
| `SET DEFAULT`（既定値の親が居ない） | 採らずに警告 |
| `RESTRICT` / `NO ACTION` | 採らずに警告 |

- `SET NULL` で null にするのは**その外部キーの全列**です（SQLite自身がそうします）。
  一部だけでは、残った列が孤児を指し続けます。向け直しの対象は主キーを指す列だけなので、
  **向け直す範囲と null にする範囲は違います**
- 1つの表が `CASCADE` の親と `SET NULL` の親を両方持つ場合、**採らないと決めた親が
  1つでもあれば採りません**（外部キーを見る順番で結果は変わりません）
- **外部キーが子の主キーを兼ねる1:1の形では、`SET NULL` も `SET DEFAULT` も適用せず、
  採らずに警告します。** SQLite は主キーでも書き換えますが、このライブラリは行を主キーで
  同定しているため追随できません（`NULL` にすると `_changelog.recordId` に触れて例外になり
  取り込みが巻き戻り、既定値を入れるとその行が**別のidの行に化けます**。どちらも実測）
- この判断が働くのは**向け直しが起きた参照**で、しかも**消えたと分かっている**
  場合だけです。単に「まだ届いていない親」を指す子は今までどおり採ります
  （取り込みは外部キーの検査を終端まで遅らせるので、親があとから同じ取り込みで届くのは
  普通に起きます）
- 「消えたと分かっている」は `_tombstone` に載っていることですが、**それだけでは足りません**。
  tombstone は「いつか消された」の記録であって「今も消えている」ではないため、
  **取り込み元にその行が現存すれば「作り直された」とみなして子を採ります**
  （`applyTombstoneDelete` が再作成された行の tombstone を無視するのと同じ物差し）。
  これが無いと、同じ取り込みの中で親が作り直されるとき、**子が先に処理された場合だけ
  子が捨てられる**という順番依存になります
- 外部キーが効いていない接続（`PRAGMA foreign_keys = OFF`）では、再現すべき動作が
  そもそも起きないので何もしません
- `SET DEFAULT` は SQLite の動作をそのまま再現します（実測にもとづく）。SQLite は
  子の外部キー列を**宣言された既定値**にし、複合外部キーでも**全列**が対象で、既定値の
  宣言が無い列は `NULL` になります。既定値の親が在れば子はそこへ繋がって生き残り、
  無ければ削除そのものが外部キー違反で失敗します。ライブラリは前者を「既定値を入れて
  採る」、後者を「採らずに警告」に対応させています（後者は「その削除は起きなかった」を
  再現する術が無いため）。
  なお `PRAGMA table_info` の `dflt_value` は**SQLの字面**（`DEFAULT 'x'` なら
  `'x'` とクォート込み、式なら式そのもの）なので、SQLite に評価させてから入れています

**見送った行・列を外した行は、`SyncResult.warnings` に載ります。** 黙っては捨てません。

```
Dropped question_scores:qs-a: parent exam_students:es-b is gone (ON DELETE CASCADE)
Kept memos:memo-1 with authorId set to NULL: parent users:u-b is gone (ON DELETE SET NULL)
```

### 意図が重なれば、片方は失われる

畳みはLWW（Last Write Wins）です。**2人が同じユニークキーの行に別々の中身を入れたとき、
残るのは片方だけ**で、負けた側の中身はどこにも残りません（列の意味はライブラリからは
分からないため、勝者が全ての列を総取りします）。

これは不具合ではなく、この設計が選んだ帰結です。隠さずに書いておきます。

- 別々の端末が同じ生徒を追加し、それぞれ別の採点を入れていた場合、生徒は1行にまとまり、
  採点は**設問ごとに**LWWで1つに決まります
- 親の畳みで消える行にぶら下がっていた子は、勝った行へ付け替えられます。ただし
  **子どうしが同じユニークキーを握っていれば、子もまた1つに畳まれます**
- 何がまとまったかは `SyncResult.folds` に、採れなかった行は `SyncResult.warnings` に
  出ます。**利用者に見せてください。** ライブラリは黙って畳みますが、黙って隠しはしません
...
以下は**現在も起きます**。どれも「同期が止まる」ものではなく、端末間で状態が揃わない
（または行が失われる）形です。

1. **畳み先がどこにも無く、消えた記録も無いとき、敗者行は消えない** — 畳み先の行が
   ローカルにもリモートにも見つからない場合、敗者行はそのまま残します（消すと子が
   道連れになるため）。結果として、敗者行を持っていた端末と持っていなかった端末で
   **その行の存在そのものが食い違います**。

   **畳み先が「消された」と記録から分かる場合は、もう起きません。** 畳まれた先が死んで
   いるならこの行も一緒に死んでいる、として**一群の死の時刻**で敗者行をLWWします。
   残るのは「その id について何も知らない」場合——畳み先の行も、その削除の記録も、
   一度も届いていないとき——だけです。

   その形が残っているあいだは、**敗者行の子も食い違ったままになります。** 敗者行を
   持っている端末はその子を持ち、畳んだ端末は `ON DELETE` に従って見送ります。
   見送りは各端末の状態としては正しい（親が無い側は子も無い）ので、この食い違いの根は
   「敗者行が消せない」ことです。**警告 `Dropped …` が出るのは見送ったその1回だけ**で、
   送り主がその子に触らなければ二度と出ません。0 でない警告はその場で利用者へ
   見せてください
2. **親の畳みが無効になっても、子の畳みは無効にならない** — 親の畳みを根拠に子どうしを
   畳んだあとで、その親の畳みが（敗者行の更新によって）無効になっても、子の畳みは
   そのまま残ります。子の記録は「どの親の畳みが原因か」を持っていないためです
   （詳細は `docs/child-fold-not-revoked.md`）
3. **同一主キー・同時刻の純粋な同点 — 解けませんが、報告します** — 同じ id の行が、
   別々の端末で**まったく同じ時刻**に別々の中身へ更新された場合、どちらも相手を
   「厳密に新しくない」として採らず、食い違ったまま残ります（別idの畳みには主キー
   辞書順の同点処理がありますが、同一主キーの経路にはありません）。

   同じ形は、**親の畳みが端末によって有効／無効に分かれたとき**にも起こります
   （同じ子が、同じ時刻のまま別々の親を指して持ち合う）。

   **どちらが正しいかはドメインの意味で決まる**ので、ライブラリは解きません。勝手に
   片方を選べば必ずどちらかの編集が消えるためです。代わりに `SyncResult.warnings` へ
   出します。
   ...
   **人へ見せてください。** どちらかの行に触れば時刻が動いて決着します
4. **`SET NULL` はユニーク制約を実質無効にする** — 消えた畳み先を指していた子の外部キーを
   `NULL` にすると、その列を含むユニークキーは衝突しなくなります（SQLiteのUNIQUEは
   NULL同士を衝突させません）。同じ「空の親」を持つ行がいくつでも並びます
5. **畳む向きが端末どうしで食い違うと、負けた向きを刻んだ端末は追いつけません —
   解けませんが、報告します** — 別idの同じユニークキーは畳みで1行にまとめますが、
   **どちらが生き残るかは、その端末が見ている版によって変わります**。片方の端末が
   `A→B` を刻んだあとに `A` が更新されて同着になると、決着（同着は生き残る id の
   辞書順）は `B→A` へ変わります。決着そのものは全端末で同じ答えになりますが、
   **すでに `A` の墓標を立ててしまった端末は、それを自分では取り消せません** ——
   取り消すと、その id の古い `DELETE` エントリが現在時刻を削除時刻として名乗り、
   他端末の生きている行を消すためです。結果として `A` を持つ端末と持たない端末に
   分かれます。

   ここで「一群ごと消す」へ寄せることもできますが、それは**誰も消していない `A` を
   消す**ことになります。逆に「`A` を生かす」へ寄せるには上の墓標の取り消しが要ります。
   利用者が敗者idを消したとき、それが「この記録を消した」のか「死んだ id を消した」
   のかは**同じ公開事実から両方に読める**ので、ライブラリには決められません
   （`sync/entries.ts` の `resolveMovedId` に、同じ形で答えが逆になる実測があります）。

   代わりに `SyncResult.warnings` へ出します。

   ```
   Stalemate on accounts:a1: another client merged a1 into a2 while this client
   merged a2 into a1, and the tie goes to a1. A client that already recorded the
   other direction cannot take a1 back, so that client may keep differing.
   Edit or delete the row on one side to break the tie.
   ```

   向きの食い違いが**収束する**場合（負けた向きを刻んだ端末がまだ何も刻んでいない形）
   でもこの文言は出ます。収束しない形だけを外から見分ける術が無く、黙って食い違うのが
   最悪だからです。決定的な再現は `__tests__/fold-crossing-deletion.test.ts` にあります

   なお、**親と主キーを共有する1:1の子（`id` が外部キーを兼ねる表）で行のidそのものが
   動く形は塞ぎました。** 動いた先への削除の読み替えは、その行を手元に持っている端末
   だけが行います（持っていない端末が読み替えると、誰にも伝わらないローカルの墓標で
   あとから届く無関係な行を永久に拒みます。実測）。
6. **`ON UPDATE` は見ていません** — 畳みが行のidを動かすのは、外部キーが子自身の主キーを
   兼ねる1:1のテーブルだけです（付け替えで子のidが親のidへ動きます）。このとき孫の
   `ON UPDATE` が働きます。`CASCADE` / `RESTRICT` / `NO ACTION` はいずれも通ります
   （検査は終端まで遅らせてあり、孫はライブラリが付け替えます）が、**`ON UPDATE SET NULL`
   で孫の参照列が `NOT NULL` の場合は例外になり、その相手ぶんの取り込みが巻き戻ります**
   （実測）。`ON DELETE` と違い、`ON UPDATE` に対する備えはありません
