...
1. **畳み先がどこにも無いとき、敗者行は消えない** — 畳み先の行がローカルにもリモートにも
   見つからない場合（例: 畳み先がそのあと消された）、敗者行はそのまま残します（消すと子が
   道連れになるため）。結果として、敗者行を持っていた端末と持っていなかった端末で
   **その行の存在そのものが食い違います**。上記「消えた畳み先」の扱いでも直りません。

   **その敗者行の子も食い違ったままになります。** 敗者行を持っている端末はその子を持ち、
   畳んだ端末は `ON DELETE` に従って見送ります（実測）。見送りは各端末の状態としては
   正しい（親が無い側は子も無い）ので、この食い違いの根は上の「敗者行が消せない」ことです。
   **警告 `Dropped …` が出るのは見送ったその1回だけ**で、送り主がその子に触らなければ
   二度と出ません。0 でない警告はその場で利用者へ見せてください
...
5. **`ON UPDATE` は見ていません** — 畳みが行のidを動かすのは、外部キーが子自身の主キーを
